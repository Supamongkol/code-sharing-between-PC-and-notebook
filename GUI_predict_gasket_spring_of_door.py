...
def predict_best_orientation(model, img_path, conf_threshold, debug_dir, save_debug=False):
    ...
    for angle, rotated_img in rotations.items():
        ...
        # เลือกจากจำนวนก่อน + ถ้าเท่ากันใช้ conf รวม
...
def run_prediction(loader=None, progress_var=None, progress=None, percent_label=None):
    ...
    wait_gasket = os.path.join(output_folder, "Waiting_gasket")
    wait_normal = os.path.join(output_folder, "Waiting")
    debug_root = os.path.join(output_folder, "debug_rotations")

    os.makedirs(wait_gasket, exist_ok=True)
    os.makedirs(wait_normal, exist_ok=True)
    ...
    for idx, img_name in enumerate(all_images, start=1):
        ...
        detections = best_result.boxes.cls.tolist() if len(best_result.boxes) > 0 else []

        predicted_labels = [labels[int(c)] for c in detections] if detections else ["No detection"]

        gasket_count = predicted_labels.count("gasket")
        spring_correct = predicted_labels.count("correct")
        spring_missing = predicted_labels.count("missing")
        spring_wrong = predicted_labels.count("wrong")

        # ---------------- LOGIC ----------------
        if gasket_count == 0:
            status = "ไม่มี gasket"
            shutil.move(output_image_path, os.path.join(wait_gasket, f"output_{img_name}"))

        else:
            if spring_correct != 8 or spring_wrong > 0 or spring_missing > 0:
                status = "ต้องตรวจสอบ"
                shutil.move(output_image_path, os.path.join(wait_normal, f"output_{img_name}"))
            else:
                status = "ปกติ"
        ...
        excel_data.append({
            ...
            "จำนวน Detection ทั้งหมด": det_count,
            "Confidence รวม": round(conf_sum, 3),
            "gasket": gasket_count,
            "spring_correct": spring_correct,
            "spring_missing": spring_missing,
            "spring_wrong": spring_wrong,
